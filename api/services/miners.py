...
class MinerService:
    ...
    async def information(user: User) -> dict:
        ...
        for worker in await headframe_api.get_miner_workers(miner_id=user.miner_id):
            all_count += 1
        ...
        expected_income, expected_income_btc = await what_to_mine_api.get_data(hash_rate=total_tera_hash_rate)
